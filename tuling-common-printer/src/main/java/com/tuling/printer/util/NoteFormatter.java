package com.tuling.printer.util;
...
public class NoteFormatter {
    ...
    private static String orderNameEmpty = StrUtil.repeat(" ", MAX_NAME_CHAR_LEN);
    ...
    public static String formatPrintOrderItemBy4Column(String foodName, Integer quantity, Double singlePrice, Double price) throws Exception {
        ...
        if (mod <= LAST_ROW_MAX_NAME_CHAR_LEN) {
            ...
            builder.append(StrUtil.repeat(" ", (MAX_NAME_CHAR_LEN - mod)));
            builder.append(quanityStr).append(StrUtil.repeat(" ", (MAX_QUANTITY_CHAR_LEN - itemQuans.length)));
            builder.append(priceStr).append(StrUtil.repeat(" ", (MAX_PRICE_CHAR_LEN - itemPrices.length)));
        } else {
            // 对菜名进行猜分
    ...
    public static String formatPrintOrderItemForNewLine80(String foodName, Integer quantity, Double price) throws Exception {
        ...
        if (mod <= LAST_ROW_MAX_NAME_CHAR_LEN80) {
            ...
            // 在同一行,留4个英文字符的空格
            //if  in the same row, fill with 4 spaces at the end of name column
            builder.append(StrUtil.repeat(" ", (MAX_NAME_CHAR_LEN80 - mod)));

            String quanityStr = quantity.toString();
            byte[] itemQuans = quanityStr.getBytes("GBK");
            String priceStr = roundByTwo(price);
            byte[] itemPrices = (priceStr).getBytes("GBK");
            builder.append(quanityStr).append(StrUtil.repeat(" ", (MAX_QUANTITY_CHAR_LEN80 - itemQuans.length)));
            builder.append(priceStr).append(StrUtil.repeat(" ", (MAX_PRICE_CHAR_LEN80 - itemPrices.length)));
        } else {
    ...
    private static String[] string2StringArray(String src, Integer length) {
        if (StrUtil.isBlank(src) || length <= 0) {
            return null;
        }
